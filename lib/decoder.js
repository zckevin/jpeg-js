...
var JpegImage = (function jpegImage() {
  ...
  constructor.prototype = {
    ...
    parse: function parse(data) {
      ...
      this.adobe = adobe;
      this.components = [];
      // use luma channel/component only.
      frame.componentsOrder.length = 1;
...
function getImageComponents(imageBuffer) {
  var defaultOpts = {
    useTArray: false,
    // "undefined" means "Choose whether to transform colors based on the image’s color model."
    colorTransform: undefined,
  ...
  var arr = new Uint8Array(imageBuffer);
  var decoder = new JpegImage();
  decoder.parse(arr);

  return decoder.components;
}

function writeDataToImage(jpegTemplateBuffer, nextByteFn) {
  var defaultOpts = {
    useTArray: false,
    // "undefined" means "Choose whether to transform colors based on the image’s color model."
    colorTransform: undefined,
  ...
  var arr = new Uint8Array(jpegTemplateBuffer);
  var decoder = new JpegImage();
  decoder.parse(arr);
  decoder.colorTransform = opts.colorTransform;

  let drained = false;
  decoder.components.map(c => {
    c.lines.map(line => {
      for (let i = 0; i < line.length; i ++) {
        let b;
        if (!drained) {
          b = nextByteFn()
          if (b == null) drained = true;
        } else {
          b = getRandomInt(255)
        }
        line[i] = b;
